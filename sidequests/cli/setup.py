...
def _register_claude_code(project_root: Path) -> None:
    ...
    # Delegate to the existing adapter setup module
    import sys
...
def _register_codex(project_root: Path) -> None:
    ...
    # Read existing TOML, inject sidequests entry
...
def _register_gemini_cli() -> None:
    ...
    # Gemini CLI MCP config (check actual path at build time)
    # Trying both known locations
    config_candidates = [
...
def _merge_mcp_config(config_path: Path, server_name: str, server_entry: dict) -> None:
    ...
    if server_name in servers:
        # Update the entry (path may have changed on reinstall)
...
def run_setup(target: str, project_root: str | None) -> None:
    ...
    # Determine which clients to register
    if target == "all":
        installed = detect_installed_clients()
        targets = [name for name, present in installed.items() if present]
        if not targets:
            print("No AI clients detected. Install Claude Code, Codex, or Gemini CLI.")
    ...
    import time
    time.sleep(2)  # Give daemon a moment to initialize
    try:
