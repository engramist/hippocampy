...
def test_wiki_status_not_generated(mock_config):
    with patch("campy.cli.main.load_config", return_value=mock_config):
        with patch("campy.cli.wiki.Path.exists", return_value=False):
...
def test_wiki_status_existing(mock_config):
    with patch("campy.cli.main.load_config", return_value=mock_config):
        with patch("campy.cli.wiki.Path.exists", return_value=True):
            with patch("campy.cli.wiki.Path.rglob", return_value=[MagicMock(), MagicMock()]):
                result = runner.invoke(app, ["wiki", "status"])
                assert result.exit_code == 0
