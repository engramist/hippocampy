...
def test_install_codex_plugin_archives_legacy_process_skills(monkeypatch, tmp_path):
    ...
    assert (skills_root / "grill" / "SKILL.md").exists()
    assert (skills_root / "diagnose" / "SKILL.md").exists()
    assert (skills_root / "handoff" / "SKILL.md").exists()
    assert (skills_root / "tdd" / "SKILL.md").exists()
    assert "name: campy-grill" in (skills_root / "grill" / "SKILL.md").read_text()
    assert "name: campy-diagnose" in (skills_root / "diagnose" / "SKILL.md").read_text()
    assert "name: campy-handoff" in (skills_root / "handoff" / "SKILL.md").read_text()
    assert "name: campy-tdd" in (skills_root / "tdd" / "SKILL.md").read_text()
