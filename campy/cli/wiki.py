...
app = typer.Typer(help="Manage the HippoCampy Wiki projection.")
...
def get_persona_config(ctx: typer.Context, persona_name: str = "default") -> dict:
    ...
    if not personas:
        ...
        return {
            ...
            "obsidian_vault_name": wiki_cfg.get("obsidian_vault_name", "HippoCampy")
        }
    ...
    for p in personas:
        if p.get("name") == persona_name:
            ...
            if "obsidian_vault_name" not in p:
                p["obsidian_vault_name"] = wiki_cfg.get("obsidian_vault_name", "HippoCampy")
            return p
...
def wiki_open(
    ...
    vault_dir = Path(p_cfg.get("vault_dir", "wiki")).resolve()
    output_dir = Path(p_cfg.get("output_dir")).resolve()
    vault_name = p_cfg.get("obsidian_vault_name", "HippoCampy")
