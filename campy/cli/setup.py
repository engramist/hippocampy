...
from campy.branding import PRIMARY_MCP_SERVER, LEGACY_MCP_SERVER, PRODUCT_NAME
...
_OPENCLAW_EXTENSION_DIR = _PROJECT_ROOT / "extensions" / "hippocampy"
...
def _register_claude_desktop() -> None:
    ...
    # B-6 Plan: Use python -m campy.adapters.claude_desktop
    entry = {
        "mcpServers": {
            PRIMARY_MCP_SERVER: {
    ...
    _merge_mcp_config(config_path, PRIMARY_MCP_SERVER, entry["mcpServers"][PRIMARY_MCP_SERVER])
...
def _register_gemini_cli() -> None:
    ...
    _merge_mcp_config(config_path, PRIMARY_MCP_SERVER, entry)
...
def _merge_mcp_config(config_path: Path, server_name: str, server_entry: dict) -> None:
    ...
    for legacy in (LEGACY_MCP_SERVER, "sidequests-brain", "sidequests-brain-desktop"):
...
def _patch_openclaw_config(config_path: Path) -> None:
    """Ensure OpenClaw trusts and surfaces the Campy plugin tools."""
    config_path.parent.mkdir(parents=True, exist_ok=True)
    config = {}
    if config_path.exists():
        try:
    ...
    if "hippocampy" not in allow:
        allow.append("hippocampy")
...
def _ensure_sidequests_toml(project_root: Path) -> None:
    """Write a default campy.toml if none exists."""
    template_path = Path(__file__).parent.parent.parent / "campy.toml"
    target_path   = project_root / "campy.toml"

    if not target_path.exists() and template_path.exists():
        shutil.copy(template_path, target_path)
        print(f"  [✓] Created campy.toml at {target_path}")
    elif target_path.exists():
        print(f"  [=] campy.toml already exists at {target_path}")
...
def _setup_daemon(project_root: Path) -> bool:
    ...
    if system == "Darwin":
        from campy.cli.launchd import write_plist, load_plist, is_loaded
...
def _setup_systemd(project_root: Path) -> bool:
    ...
    service_path = service_dir / "hippocampy.service"
    service_dir.mkdir(parents=True, exist_ok=True)

    daemon_script = shutil.which("campy-daemon") or str(
...
"""
    ...
    result = subprocess.run(
        ["systemctl", "--user", "enable", "--now", "hippocampy.service"],
...
def run_setup(target: str, project_root: str | None) -> None:
    ...
    if target == "all":
        ...
        if not targets:
            ...
            print("You can also specify a target directly: campy setup --target claude-code")
        else:
    ...
    try:
        ...
        print("      Try `campy status` once the daemon is running.")
