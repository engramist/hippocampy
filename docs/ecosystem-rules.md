...
**Rule (B314) — new code MUST use named queries; inline Cypher outside the allowlist is rejected.** `campy/brain/hippocampus/graph/gateway.py`'s `GraphGateway`/`NamedQuery`/`QueryRegistry` is the sanctioned path from application code to KuzuDB. New code MUST reach the database through `GraphGateway.run("<domain>.<verb>_<subject>", **params)` against a `NamedQuery` registered under `campy/brain/hippocampus/graph/queries/`, not by building Cypher strings and passing them to `KuzuClient.execute()`/`execute_read()`/`execute_write()` directly. `scripts/check_cypher_ratchet.py` (`make check-cypher`, wired into CI) enforces this mechanically: it fails if the count of inline-Cypher lines outside the allowlist — or the count of `GraphGateway.execute_raw()` escape-hatch call sites — increases relative to `scripts/cypher_baseline.json`. The allowlist is narrow and reasoned, not a loophole: `schema.py`'s DDL and `kuzu_client.py`'s own engine plumbing (both inherently engine-specific, see `docs/ARCHITECTURE.md`'s B314 section), and `graph/queries/**` (where migrated Cypher is supposed to live). Everything else — including code in files not yet migrated — is expected to add new queries as `NamedQuery` entries rather than growing the pool of inline Cypher those files already carry; `execute_raw()` exists for the rare case a new call site cannot yet be expressed as a static named query, but every use is logged, ratchet-counted, and expected to be temporary.

**Non-negotiable (B315) — tenant and workspace are derived from the transport credential, never from request params.** `campy/brain/auth.py`'s `TransportContext` carries only what the transport itself attests to (peer credentials, TLS identity, an observability-only client hint) — it has no field a JSON-RPC request body could populate, and it is constructed in `brain_daemon.py`'s `_handle_connection` before a single byte of the request is parsed. The `Principal` resolved from it (via `PrincipalResolver`) is what carries `tenant_id`/`workspace_id` to a handler, never a `params` dict the caller controls. This is enforced twice, deliberately redundantly: structurally, by `TransportContext` having nowhere to put request data even if someone tried; and mechanically, by `_dispatch`'s forbidden-key guard, which rejects any request whose `params` contains `tenant_id`, `workspace_id`, `subject_id`, `principal`, or `scopes` with JSON-RPC `-32602` and a WARNING-level log naming the offending key. If an agent could pass `workspace_id` as an ordinary argument, any agent could read another tenant's memory — a textbook confused-deputy vulnerability, and one prompt injection away from being exercised with an LLM in the loop. Local single-user Campy is not exempt: `LocalSingleUserResolver` always yields a real `Principal` (tenant `local`, workspace `local`, every scope) — never `None` and never a special-cased branch — so the same code path cloud deployments will use is exercised by every local test run, not just in production. See `docs/ARCHITECTURE.md`'s B315 section for the full contract.
