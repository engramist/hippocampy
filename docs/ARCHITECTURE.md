...
| Evidence so far | Warm RSS (model loaded **and invoked**, matching a running daemon), measured as a **clean install** behaves: baseline Python 18.0 MB; + spaCy loaded and invoked 388.1 MB; + fastembed loaded and invoked 595.3 MB. **Corrected 2026-09-05:** an earlier revision of this row reported 14.5 / 541.0 / 818.6 MB, measured in a dev venv holding a stale `torch` left by `sentence-transformers` (replaced by fastembed in B355). `torch` is not declared in `pyproject.toml` or `requirements.txt` and `thinc` does not require it — `thinc.compat` merely attempts `import torch` opportunistically. Those figures overstated production RSS by ~223 MB. `step1_ner.py`'s own docstring states spaCy NER "misses most software/tech terms... noun chunks catch them." B387's separate ONNX-replacement measurement (entity-span F1 0.534 vs. spaCy, 46 sentences) cannot settle this question — it measures agreement between two candidate generators, not correctness against ground truth. |
...
| Evidence so far | Warm (model loaded **and invoked**) RSS, clean-install conditions: baseline Python 18.0 MB → + spaCy loaded and invoked 388.1 MB → + fastembed loaded and invoked **595.3 MB**. (Corrected 2026-09-05 — see the row above; the original 818.6 MB figure included a stale `torch` that a clean install does not have.) Contrast with the import-time-only figures that produced the original target: B387's table (`bare Python` 14.8 MB, `+ kuzu` 24.3 MB, `+ kuzu + fastembed` 76.1 MB) and B389's table (same shape) measure imports only, before any model is invoked — calling `.embed()` alone reaches ~266–277 MB per the design doc. `<80 MB` warm is not achievable while ONNX embeddings run in-process; the design doc recommends restating any future target against the measured 595.3 MB clean-install warm baseline rather than carrying `<80 MB` forward. |
